...
def load_CIFAR_batch(file_path):
    ...
    data = np.array(data).reshape(10000, 3, 32, 32).astype("float")
...
def load_CIFAR10(file_dir):
    ...
    for filename in train_list:
        ...
        xs.append(data)
        ys.append(labels)
    x_train = np.concatenate(np.array(xs))
    y_train = np.concatenate(np.array(ys))
...
def get_CIFAR10_test_data(cifar_dir, normalize=True):
